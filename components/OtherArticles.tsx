...
export default function OtherArticles() {
    const { data } = useRequestWithQuery("/articles", {
        limit: "3",
        page: "1"
    });
    ...
    return (
        <div>
            <p className="text-xl text-slate-900 font-bold">Other articles</p>
            <div className="grid grid-cols-1 xl:grid-cols-3 xl:space-x-10 xl:px-[20px] pt-4 xl:space-y-10 space-y-6">
                {articles.map((x: any, index: string) => (
                    <div key={index} onClick={() => console.log(x)} className="cursor-pointer">
